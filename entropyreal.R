library('MASS')
library(foreach)
library(doParallel)
library('Rfast')

#minrenyidata, renyidata, tsallidata, shanondata are selected cell, gene matrix
#darmanisc, pollenc, kleinc are cell types
#darmanisd, pollenc, kleind are original data
#renyidata and tsallisdata are for two q values.
#There are two rentidata03, renyidata07 and two tsallisdata05,  tsallisdata07 for each data 

#setwd("C:/Users/SR/Desktop/cvpr_supervised_sc3/pollenfin")
#ProcessedData = read.table('pollen_process.csv', header=FALSE,sep=',')
#ProcessedData=t(ProcessedData)
#OriginalAnnotations = read.table("pollen_annotation.csv", header = FALSE, sep = ',')
load("scrnadata_entropy.RData")

jointprob_3 <-function(u) {
  ncol=length(unique(u[,1]))
  nrow=length(unique(u[,2]))*length(unique(u[,3]))
  probtable=matrix(0,nrow,ncol)
  a=unique(u[,1])
  for(i in 1:length(a))
  {
    index=which(u[,1]==a[i])
    pmat=as.matrix(u[index,2:3])
    b=unique(pmat[,1])
    count=1
    for(j in 1: length(b))
    {
      index1=which(pmat[,1]==b[j])
      pmat1=pmat[index1,2]
      s1=aggregate(data.frame(count = pmat1), list(value = pmat1), length)
      lens1=length(s1$count)
      probtable[(count:(count+lens1-1)),i]=(s1$count)/nrow(data)
      count=count+lens1
    }
  }
  probtable
}

jointprob_2 <-function(u) {
  ncol=length(unique(u[,1]))
  nrow=length(unique(u[,2]))
  probtable=matrix(0,nrow,ncol)
  a=unique(u[,1])
  for(i in 1:length(a))
  {
    index=which(u[,1]==a[i])
    pmat1=as.matrix(u[index,2])
    s1=aggregate(data.frame(count = pmat1), list(value = pmat1), length)
    lens1=length(s1$count)
    probtable[1:lens1,i]=(s1$count)/nrow(data)
  }  
  probtable
}

data<-as.matrix(darmanisd)
classc<-as.matrix(darmanisc)
n <- nrow(data)
col<-ncol(data)
count=ncol(data)
nf=50
renifea=matrix(0,2,nf)
minrenifea=matrix(0,1,nf)
shanonfea=matrix(0,1,nf)
tsallisfea=matrix(0,2,nf)



# Renyi min entropy
fea<- matrix(0, nrow=1,ncol =nf)
cl <- makeCluster(50)
registerDoParallel(cl)
parl<-foreach(j=1:count, .combine=c,.packages= c("Rfast")) %dopar%
{
  u<-as.matrix(cbind(classc,data[,j]))
  tmat=jointprob_2(u)
  transvecfea=-(log(sum(rowMaxs(tmat,value=TRUE))))
}

mimc1<-as.matrix(parl)
fea[1,1]=which.max(mimc1)
idx=which.max(mimc1)
#fea[1,1]<-which(mimc1==max(mimc1[which(mimc1>0)]))[1]
#idx=which(mimc1==min(mimc1[which(mimc1>0)]))[1]

for(i in 1:(nf-1))
{  
  parl<-foreach(j=1:count, .combine=c,.packages= c("Rfast")) %dopar%
  {
    u2<-as.matrix(cbind(classc,data[,idx],data[,j]))
    tmat=jointprob_3(u2)
    feat=-(log(sum(rowMaxs(tmat,value=TRUE))))
  }
  feat<-as.matrix(parl)
  feat[fea[1:i]]<-Inf
  idx=which(feat==min(feat[which(feat>0)]))[1]
  fea[1,(i+1)]=idx
}
minrenifea[1,]=fea
stopCluster(cl)
minrenyidata=data[,fea]


# Renyi entropy with q
cl <- makeCluster(50)
registerDoParallel(cl)
for(m in 1:2)
{qindex=c(0.3,0.7)
q=qindex[m]
fea<- matrix(0, nrow=2,ncol =nf)
parl<-foreach(j=1:count, .combine=c,.packages= c("Rfast")) %dopar%
{
  u<-as.matrix(cbind(classc,data[,j]))
  tmat=jointprob_2(u)
  tmat=tmat^q
  transvecfea=(q/(1-q))*(log(sum((rowsums(tmat))^(1/q))))
}

mimc1<-as.matrix(parl)
fea[1,1]=which.max(mimc1)
idx=which.max(mimc1)
#fea[1,1]<-which(mimc1==max(mimc1[which(mimc1>0)]))[1]
#idx=which(mimc1==min(mimc1[which(mimc1>0)]))[1]

for(i in 1:(nf-1))
{  
  parl<-foreach(j=1:count, .combine=c,.packages= c("Rfast")) %dopar%
  {
    u2<-as.matrix(cbind(classc,data[,idx],data[,j]))
    tmat=jointprob_3(u2)
    tmat=tmat^q
    transvecfea=(q/(1-q))*(log(sum((rowsums(tmat))^(1/q))))
  }
  feat<-as.matrix(parl)
  feat[fea[1:i]]<-Inf
  idx=which(feat==min(feat[which(feat>0)]))[1]
  fea[1,(i+1)]=idx
  print(i)
}
renifea[m,]=fea
}
stopCluster(cl)
f1=renifea[1,]
renyidata03=data[,f1]
f1=renifea[2,]
renyidata07=data[,f1]

# Tsallis Entropy
cl <- makeCluster(50)
registerDoParallel(cl)
for(m in 1:2)
{qindex=c(0.3,0.5)
q=qindex[m]
fea<- matrix(0, nrow=1,ncol =nf)
parl<-foreach(j=1:count, .combine=c,.packages= c("Rfast")) %dopar%
{
  u<-as.matrix(cbind(classc,data[,j]))
  tmat=jointprob_2(u)
  tmatq=tmat^q
  nem=sum(tmatq)
  s1=aggregate(data.frame(count = data[,j]), list(value = data[,j]), length)
  lens1=length(s1$count)
  probnew=(s1$count)/nrow(data)
  denom=sum(probnew^q)
  trans=(1/(q-1))*(1-(nem/denom))
}

mimc1<-as.matrix(parl)
fea[1,1]=which.max(mimc1)
idx=which.max(mimc1)
#fea[1,1]<-which(mimc1==max(mimc1[which(mimc1>0)]))[1]
#idx=which(mimc1==min(mimc1[which(mimc1>0)]))[1]

for(i in 1:(nf-1))
{  
  parl<-foreach(j=1:count, .combine=c,.packages= c("Rfast")) %dopar%
  {
    u2<-as.matrix(cbind(classc,data[,idx],data[,j]))
    tmat=jointprob_3(u2)
    tmatq=tmat^q
    nem=sum(tmatq)
    u3<-as.matrix(cbind(data[,idx],data[,j]))
    probnew=jointprob_2(u3)
    denom=sum(probnew^q)
    trans=(1/(q-1))*(1-(nem/denom))
  }
  feat<-as.matrix(parl)
  feat[fea[1:i]]<-Inf
  idx=which(feat==min(feat[which(feat>0)]))[1]
  fea[1,(i+1)]=idx
}
tsallisfea[m,]=fea
}
stopCluster(cl)
f1=tsallisfea[1,]
tsallisdata03=data[,f1]
f1=tsallisfea[2,]
tsalisdata05=data[,f1]


# Shanon Entropy
cl <- makeCluster(50)
registerDoParallel(cl)
fea<- matrix(0, nrow=1,ncol =nf)
parl<-foreach(j=1:count, .combine=c,.packages= c("Rfast","GLDEX")) %dopar%
{
  u<-as.matrix(cbind(classc,data[,j]))
  tmat=jointprob_2(u)
  tmat=as.vector(tmat)
  tmat=fun.zero.omit(tmat)
  ent1=-sum(tmat*log2(tmat))
  s1=aggregate(data.frame(count = data[,j]), list(value = data[,j]), length)
  lens1=length(s1$count)
  probnew=(s1$count)/nrow(data)
  ent2=-sum(probnew*log2(probnew))
  trans=ent1-ent2
}

mimc1<-as.matrix(parl)
fea[1,1]=which.max(mimc1)
idx=which.max(mimc1)
#fea[1,1]<-which(mimc1==max(mimc1[which(mimc1>0)]))[1]
#idx=which(mimc1==min(mimc1[which(mimc1>0)]))[1]

for(i in 1:(nf-1))
{  
  parl<-foreach(j=1:count, .combine=c,.packages= c("Rfast","GLDEX")) %dopar%
  {
    u2<-as.matrix(cbind(classc,data[,idx],data[,j]))
    tmat=jointprob_3(u2)
    tmat=as.vector(tmat)
    tmat=fun.zero.omit(tmat)
    ent1=-sum(tmat*log2(tmat))
    u3<-as.matrix(cbind(data[,idx],data[,j]))
    probnew=jointprob_2(u3)
    probnew=as.vector(probnew)
    probnew=fun.zero.omit(probnew)
    ent2=-sum(probnew*log2(probnew))
    trans=ent1-ent2
  }
  feat<-as.matrix(parl)
  feat[fea[1:i]]<-Inf
  idx=which(feat==min(feat[which(feat>0)]))[1]
  fea[1,(i+1)]=idx
}
shanonfea[1,]=fea
registerDoSEQ()
shanondata=data[,fea]

save.image("darmanisresult.RData")
